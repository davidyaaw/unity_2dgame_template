...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
        ...
        void Start()
		{
            ...
            m_footstepAudio.spatialBlend = 1f;  // Устанавливает пространственный бленд на 1, чтобы звук был трехмерным

			m_footstepAudio.clip = footstepSound;

            m_jumpAudio = gameObject.AddComponent<AudioSource>();
            m_jumpAudio.spatialBlend = 1f;

			m_jumpAudio.clip = jumpSound;

            m_dashAudio = gameObject.AddComponent<AudioSource>();
            m_dashAudio.spatialBlend = 1f;

			m_dashAudio.clip = dashSound;
		}
		...
		private void FixedUpdate()
		{
			...
			if (isCurrentlyPlayable)
			{
                ...
                if (isGrounded && Mathf.Abs(m_rb.velocity.x) > 0.1f && !m_footstepAudio.isPlaying)
                {
